...
def mart_loss_dummy(P, model,
    ...
    labels_oh = F.one_hot(y, num_classes=DUMMY_NUM_CLASS)
    labels_oh_dummy = F.one_hot(y + int(DUMMY_NUM_CLASS / 2), num_classes=DUMMY_NUM_CLASS)
    ...
    data_dummy = torch.cat((x_natural, x_adv))
    ...
    logits_dummy = model(data_dummy)
    loss_adv = F.cross_entropy(logits_dummy, labels_dummy) + F.nll_loss(torch.log(1.0001 - adv_probs + 1e-12), new_y)
    # loss_adv = F.cross_entropy(logits_adv, y) + F.nll_loss(torch.log(1.0001 - adv_probs + 1e-12), new_y)
